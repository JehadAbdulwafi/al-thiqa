    ...
    return (
      <div className={cn('flex flex-row items-center gap-x-2 justify-center', className)}>
        ...
        <p className="text-primary text-sm">
          تم الرفع بنجاح {files.length} ملف{files.length > 1 ? 'ات' : ''}
        </p>
      </div>
    )
  }
    ...
    <div className={cn('flex flex-col', className)}>
          ...
          >
            ...
            <div className="shrink grow flex flex-col items-start truncate">
              ...
              {file.errors.length > 0 ? (
                ...
                <p className="text-xs text-destructive">فشل الرفع: {fileError.message}</p>
      ...
      {exceedMaxFiles && (
        <p className="text-sm text-left mt-2 text-destructive">
          يمكنك رفع ما يصل إلى {maxFiles} ملف، يرجى إزالة {files.length - maxFiles} ملف
          {files.length - maxFiles > 1 ? 'ات' : ''}.
        </p>
      )}
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn('flex flex-col items-center gap-y-2', className)}>
      ...
      <p className="text-sm">
        رفع{!!maxFiles && maxFiles > 1 ? ` ${maxFiles}` : ''} ملف
        {maxFiles === 1 ? '' : 'ات'}
      </p>
